...
function showProc(c, a) {
	...
	if(a) {
		...
		$("<img src='" + SKIN_PATH + "img/processing.gif' id='imgProc' alt='æ­£åœ¨å¤„ç†' />").insertAfter(c)
	} else {
		$(c).show();
		b.remove()
	}
	...
	try {
		...
		if(window.netscape) {
			try {
				...
				alert("æŠ±æ­‰ï¼æ‚¨çš„æµè§ˆå™¨ä¸æ”¯æŒç›´æŽ¥è®¾ä¸ºé¦–é¡µã€‚è¯·åœ¨æµè§ˆå™¨åœ°å€æ è¾“å…¥â€œabout:configâ€å¹¶å›žè½¦ç„¶åŽå°†[signed.applets.codebase_principal_support]è®¾ç½®ä¸ºâ€œtrueâ€ï¼Œç‚¹å‡»â€œåŠ å…¥æ”¶è—â€åŽå¿½ç•¥å®‰å…¨æç¤ºï¼Œå³å¯è®¾ç½®æˆåŠŸã€‚")
			}
			var a = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefBranch);
			a.setCharPref("browser.startup.homepage", d)
		} else {
			alert("æŠ±æ­‰ï¼Œæ‚¨çš„æµè§ˆå™¨ä¸æ”¯æŒè‡ªåŠ¨è®¾ç½®é¦–é¡µ, è¯·ä½¿ç”¨æµè§ˆå™¨èœå•æ‰‹åŠ¨è®¾ç½®!")
		}
	}
};
...
function Marquee() {
	...
	if(!this.ID) {
		alert('æ‚¨è¦è®¾ç½®çš„"' + arguments[0] + '"åˆå§‹åŒ–é”™è¯¯\r\nè¯·æ£€æŸ¥æ ‡ç­¾IDè®¾ç½®æ˜¯å¦æ­£ç¡®!');
		this.ID = -1;
		return
	}
...
function ScrollPicleft(b, g, f, e) {
	...
	this.initialize = function() {
		if(!this.scrollContId) {
			throw new Error("å¿…é¡»æŒ‡å®šscrollContId.");
			return
		}
		this.scrollContDiv = sina.$(this.scrollContId);
		if(!this.scrollContDiv) {
			throw new Error('scrollContIdä¸æ˜¯æ­£ç¡®çš„å¯¹è±¡.(scrollContId = "' + this.scrollContId + '")');
			return
		}
...
//åŠ è½½jsï¼Œsrcä¸ºæ–‡ä»¶è·¯å¾„ï¼Œfnä¸ºå›žè°ƒå‡½æ•°ï¼Œdocä¸ºå½“å‰æ–‡æ¡£
 ...
 * v2.1.1ï¼šä¿®å¤å½“è°ƒç”¨å¤šä¸ªSuperSlideï¼Œå¹¶è®¾ç½®returnDefault:true æ—¶è¿”å›ždefaultIndexç´¢å¼•é”™è¯¯
 */
! function(a) {
...
function PostUsersInfo(a) {
	var h = "";
	var i = "é¦–é¡µåœ¨çº¿ç•™è¨€æäº¤ä¿¡æ¯";
	var d = $v("txtName");
	var c = $v("txtTel");
	var g = $v("txtEmail");
	var add = $v("txtAddress");
	var e = $v("txtShortDesc");
	var vercode = $v("txtCmtVerCode");
	var b = "";
	if(d == "") {
		b += "<li>å§“åä¸å¯ä¸ºç©º</li>"
	}
	if(c == "") {
		b += "<li>æ‰‹æœºå·ç ä¸å¯ä¸ºç©º</li>"
	} else {
		var mpvar = /^1[3-8]\d{9}$/;
		if(!mpvar.test(c)) {
			b += "<li>æ‰‹æœºå·ç æ ¼å¼é”™è¯¯</li>"
		}
	}
	if(vercode == "") {
		b += "<li>éªŒè¯ç ä¸å¯ä¸ºç©º</li>"
	}
	if(b.length > 0) {
		$a(b);
		return
...
function changeValicode() {
	$("#imgvalidate").attr("src", '/Tools/ValidCodes.aspx?' + new Date().getTime());
}

function imgchange() {
	var img = document.getElementById("imgvalidate");
	img.src = '/Tools/ValidCodes.aspx?t=' + Math.random();
}